...
# Make loopback allowed by default and still honor local nets and your 'lan' list
ESL_APPLY_ACL="${ESL_APPLY_ACL:-loopback.auto,localnet.auto,lan}"
# Optional comma-separated extra CIDRs to allow in 'lan' (e.g. "172.17.0.0/16,192.168.50.0/24")
...
# FusionPBX Lua scripts (fixes app.lua errors)
SCRIPTS_DIR="${SCRIPTS_DIR:-/var/www/fusionpbx/app/scripts}"

# 1 to enable external profile by renaming external.xml.noload -> external.xml
...
sed_escape(){ printf '%s' "$1" | sed 's/[\/&]/\\&/g'; }  # escape / and & for sed
...
detect_bridge_cidr() {
  # best-effort: the primary interface in containers is usually eth0
  if command -v ip >/dev/null 2>&1; then
...
if [[ $have_xmlstarlet -eq 1 ]]; then
  xmlstarlet ed -L \
    ...
    "$VARS_XML"
  # ensure scripts_dir is present / correct
  if grep -q 'scripts_dir=' "$VARS_XML"; then
    xmlstarlet ed -L \
      -u "/document/section/X-PRE-PROCESS[contains(@data,'scripts_dir=')]/@data" -v "scripts_dir=$SCRIPTS_DIR" \
      "$VARS_XML"
  else
    sed -i "0,/<\/section>/s//<X-PRE-PROCESS data=\"scripts_dir=${SCRIPTS_DIR}\" \/>\n&/" "$VARS_XML"
  fi
else
  # sed path with safe escaping
  ...
  sd_scripts=$(sed_escape "$SCRIPTS_DIR")
  sed -i \
  ...
  if grep -q 'scripts_dir=' "$VARS_XML"; then
    ...
    sed -i "0,/<\/section>/s//<X-PRE-PROCESS data=\"scripts_dir=${sd_scripts}\" \/>\n&/" "$VARS_XML"
  fi
fi

# ===================== make sure sofia includes sip_profiles =====================
...
if [[ -f "$ESL_XML" ]]; then
  # enable, bind, password, and apply-inbound-acl
  sed -i \
...
# Harden/extend ACL 'lan' to include loopback, local IP, docker bridge, and extras
if [[ -f "$ACL_XML" ]]; then
  # Create lan list if missing
  if ! grep -q '<list name="lan"' "$ACL_XML"; then
  ...
  add_allow_cidr() {
    ...
    if ! grep -q "cidr=\"$cidr\"" "$ACL_XML"; then
      sed -i "/<list name=\"lan\"/,/<\/list>/ s#</list>#  <node type=\"allow\" cidr=\"$cidr\"/>\n  </list>#g" "$ACL_XML"
    fi
  }

  add_allow_cidr "127.0.0.1/32"
  add_allow_cidr "::1/128"

  # LOCAL_IP_V4 as /32 if it's not 127.0.0.1
  if [[ "$LOCAL_IP_V4" != "127.0.0.1" ]]; then
    add_allow_cidr "${LOCAL_IP_V4}/32"
  fi

  # docker/bridge CIDR if detected
  if [[ -n "$BRIDGE_CIDR" ]]; then
    add_allow_cidr "$BRIDGE_CIDR"
  fi

  # optional extras
  IFS=',' read -r -a _extra_cidrs <<< "$EXTRA_ESL_ALLOW_CIDRS"
  for c in "${_extra_cidrs[@]}"; do
    c_trim="$(echo "$c" | xargs || true)"
    [[ -n "$c_trim" ]] && add_allow_cidr "$c_trim"
  done
fi

# ===================== lua.conf.xml script-directory (fix app.lua not found) =====================
if [[ -f "$LUA_XML" ]]; then
  if grep -q 'script-directory' "$LUA_XML"; then
    sed -i "s#name=\"script-directory\" value=\"[^\"]*\"#name=\"script-directory\" value=\"${SCRIPTS_DIR}/resources/scripts\"#g" "$LUA_XML"
  else
    sed -i "/<\/settings>/i \    <param name=\"script-directory\" value=\"${SCRIPTS_DIR}/resources/scripts\"/>" "$LUA_XML"
  fi
fi

# ===================== event socket password also in vars.xml (already set above) =====================

# ===================== ensure mod_sofia autoloads =====================
if [[ -f "$MODULES_XML" ]] && ! grep -q 'mod_sofia' "$MODULES_XML"; then
  sed -i '/<\/modules>/i \  <load module="mod_sofia"/>' "$MODULES_XML"
fi

# ===================== reload & start profiles (best-effort) =====================
if command -v fs_cli >/dev/null 2>&1; then
  # after ACL fix, fs_cli should be able to connect on 127.0.0.1
...
echo "  SCRIPTS_DIR=${SCRIPTS_DIR}  ENABLE_EXTERNAL=${ENABLE_EXTERNAL}"
...
# hand control back to caller (entrypoint usually exec's FS)
exec "$@"
